...
public class CURP {
	...
	public static void main(String args[])
	{
		...
		//if( (args.length != 0) && (args[0].equalsIgnoreCase(MTOS_BD)) )
		if( args.length != 0 )
		{
			if(args[0].equalsIgnoreCase(MTOS_BD))
			{
				if( args.length > 1 )
				{
			...
			else if (args[0].equalsIgnoreCase(ALUM_BD))
			{
				if( args.length > 1 )
				{
					month = Integer.parseInt(args[1]);
			...
			else if (args[0].equalsIgnoreCase(CURPS_CHECK))
			{
				ITRDataSourceFebJun2015 ds2015 = new ITRDataSourceFebJun2015();
				checkCurps(ds2015);
			}
		}
		else
	...
	private static void printBDAlumnos(ITRDataSourceFebJun2015 ds) {
		ArrayList<ViewAlumno> alumnos = getCurerntMonthCurps(ds);
	...
	private static void printBDMTOS(ITRDataSourceFebJun2015 ds) 
	{
		getCurerntMonthCurpsMtos(ds);
	...
	private static ArrayList<ViewAlumno> getCurerntMonthCurps(ITRDataSourceFebJun2015 ds)
	{
		if(month == -1)
		{
		...
		return getCurerntMonthCurps(ds, month);
	}

	private static ArrayList<ViewAlumno> getCurerntMonthCurps(ITRDataSourceFebJun2015 ds, int currMonth)
	{
	...
	private static void checkCurps(ITRDataSourceFebJun2015 ds)
	{
		Connection conn = null;
	...
	private static void /*ArrayList<ViewMaestro>*/ getCurerntMonthCurpsMtos(ITRDataSourceFebJun2015 ds)
	{
		Connection conn = null;
			...
			try {
				...
				try {
					...
					while (rs.next()) {
						...
						id = rs.getInt(CURP.COL_00_ID);

						a.setId(id);
						a.setNombre(rs.getString(CURP.COL_01_NAME));
						a.setEspecialidad(rs.getString(CURP.COL_02_ESP));
						a.setSemestre(rs.getString(CURP.COL_03_SEM));
						a.setGrupo(rs.getString(CURP.COL_04_GPO));
						a.setGrupoId(rs.getInt(CURP.COL_05_GPO_ID));
						a.setGrupoLiteral(rs.getString(CURP.COL_06_GPO_LIT));
						a.setCurp(rs.getString(CURP.COL_07_CURP));
						a.setSemestre_ID(rs.getInt(CURP.COL_08_SEM_ID));
						a.setCodigo(rs.getString(CURP.COL_09_COD));
			...
			try {
				...
				try {
					...
					while (rs.next()) {
						...
						id = rs.getInt(CURP.MTO_00_ID);

						a.setId(id);
						a.setNombre(rs.getString(CURP.MTO_01_NAME));
						a.setRfc(rs.getString(CURP.MTO_02_RFC));
						a.setCurp(rs.getString(CURP.MTO_03_CURP));
						a.setEmail(rs.getString(CURP.MTO_04_EMAIL));
						a.setFecha_nacimiento(rs.getDate(CURP.MTO_05_FECHNAC));
						a.setFecha_Ingreso(rs.getDate(CURP.MTO_06_FECHING));
